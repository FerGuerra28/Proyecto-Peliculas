...
if($row){
    echo "<script>alert('El correo ingresado ya se encuentra registrado. Ingrese uno diferente.')
    ...
    <div style=''>
        <h2>¡Bienvenido(a) ".$_POST["txtnom"]." ".$_POST["txtape"]." a la comunidad de PurOcio!, </h2>

        <p style='font-size:17px;'> Esperamos que disfrutes tu estadía en la web. Nos esforzamos por mejorar cada día para llevarles un buen servicio y disfrutamos, al igual que ustedes, de series y películas. No dudes en compartir tus gustos, opiniones, comentarios y sugerencias.</p>
    ...
    $mail->SMTPAuth = true;
    $mail->Username = 'PurOcioWeb@gmail.com'; // email
    $mail->Password = 'PurOcio3000'; // password
    $mail->setFrom('PurOcio@gmail.com', 'PurOcio.com'); // From email and name
    $address = filter_var($_POST["txtcorreo"], FILTER_VALIDATE_EMAIL);
...
//quitar
   // if(!$mail->send()){
        //echo "Mailer Error: " . $mail->ErrorInfo;
            ...
            echo "<script>alert('Algo falló. Inténtelo en unos minutos')
...
<body style='background: #FFFFFF;font-family: Verdana; font-size: 14px;color:#1c1b1b;'>
<div style=''>
    <h2>Estimado(a) ".$row["firstname"]." ".$row["lastname"].", </h2>
    <p style='font-size:17px;'> Tu contraseña es:
    ".$row["4"].", recuerda que puedes cambiarla en el Menú de Configuración de Usuario/Cambiar contraseña</p>
  <br>
</html>"
;
...
$mail->addAddress($address, $address); // to email and name
$mail->Subject = 'Reenvío de contraseña';
$mail->msgHTML($mensaje); //$mail->msgHTML(file_get_contents('contents.html'), __DIR__); //Read an HTML message body from an external file, convert referenced images to embedded,
$mail->AltBody = 'HTML messaging not supported'; // If html emails is not supported by the receiver, show this body
// $mail->addAttachment('images/phpmailer_mini.png'); //Attach an image file
///quitar
//if(!$mail->send()){
    //echo "Mailer Error: " . $mail->ErrorInfo;
        ...
        echo "<script>alert('La contraseña ha sido enviada a su correo, por favor, revise su bandeja.')
        document.location=('../vista/login.php')</script>";
        ...
        echo "<script>alert('El correo no se encuentra registrado.')
        document.location=('../vista/recuperarcontrasenia.php')</script>";
