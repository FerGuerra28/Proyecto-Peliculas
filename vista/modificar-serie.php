...
<div class="panel-heading"><h3 style="text-align:center;">Modifica el registro de la serie</h3></div>
</div>
                <?php
                $codserie=$_REQUEST["idserie"];
                $serie=new Serie();
                $serie->setId($codserie);
                $r = $serie->getSeriebyCod();
                ?>

<form class="form-horizontal" action="../controlador/seriecontrolador.php" method="post" enctype="multipart/form-data" data-toggle="validator">

                   <input value="<?=$r[0]?>" type="hidden" name="idserie" />
        ...
        <!-- Text input-->
        <div class="form-group">
          <label class="col-md-4 control-label" for="episodes" >Episodios : </label>
          <div class="col-md-2">
          <input value="<?=$r[4]?>" id="episodes" name="txtepisodes" type="text" class="form-control input-md" onkeypress='return validaNumericos(event)' required>
          </div>
         <div class="help-block with-errors"></div>
        </div>

        <div class="form-group">
          <label class="col-md-4 control-label" for="seasons" >Temporadas: </label>
          <div class="col-md-2">
          <input value="<?=$r[5]?>" id="seasons" name="txtseasons" type="text" class="form-control input-md" onkeypress='return validaNumericos(event)' required>
          </div>
         <div class="help-block with-errors"></div>
        </div>
        ...
        <button class="btn btn-success" block="true" type="submit" name="action" value="modificar"> Guardar </button>
          <a class='btn btn-info' href='detalleSerie.php'>Cancelar</a>
          </div>
        </div>
